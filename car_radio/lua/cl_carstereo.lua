...
local cvar_outside_dist = CreateConVar("wyozicr_outsidecar_distance", "1000", FCVAR_ARCHIVE)
...
hook.Add("Think", "WCR_RadioUpdater", function()
	...
	for _,car in pairs(wyozicr.GetCarEnts()) do
		...
		if isPlaying and not shouldPlay then
			MsgN("[UnliRP] Начал воспроизводить #" .. station .. " (car: " .. tostring(car) .. " - url: " .. tostring(stereourl) .. ")")
			...
			if is_wdj then
				...
				if IsValid(master) then
					...
					if link ~= "" and link ~= "--loading" then
						MsgN("[UnliRP] Начал воспроизводить #" .. station .. " (car: " .. tostring(car) .. " - url: " .. tostring(link) .. ")")
