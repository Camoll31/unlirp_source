...
local function GetAngPos_Experimental(ply, seat, veh)
	...
	local pos = mid_local
	pos.x = pos.x - 4 -- To center the FUI we use an arbitrary number cuz yolo
...
local bg_color = Color(41, 41, 41, 200)
local border_color = Color(58, 62, 66)
...
local function CreateCarFUI()
	...
	do
		...
		do
			local lbl = subpane:Add("Label", "Магнитола", "FloatySmall", nil, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			lbl:SetPos(250, 35)
		end
		...
		do
			...
			btn:SetPos(130, 110)
			btn:SetSize(240, 100)
			btn.Paint = CreateBtnPainter(Color(255, 255, 255, 20), nil, "Переключить")
			btn.DoClick = function()
				net.Start("wyozicr_cradio")
					net.WriteInt(0, 8)
				net.SendToServer()
...
hook.Add("PostDrawTranslucentRenderables", "WCR_FUIRenderer", function()
	...
	local cur_station = veh:GetNWInt("wcr_station", 0)
	...
	do
		...
		if stereochan == "" then
			...
			if station then
				...
				stereochan = "Выкл. Нажмите 'Переключить'."
			end
		end
	end
